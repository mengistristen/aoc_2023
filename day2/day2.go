...
func (d Day2) PartOne(ch chan string) {
	...
	if sum, err := d.ProcessPartOne("./input/day2_example.txt"); err == nil {
		ch <- fmt.Sprintf("Example output: %d\n", sum)
	} else {
		ch <- fmt.Sprintf("error processing part one example: %v\n", err)
	}

	if sum, err := d.ProcessPartOne("./input/day2.txt"); err == nil {
...
func (d Day2) PartTwo(ch chan string) {
	...
	if sum, err := d.ProcessPartTwo("./input/day2_example.txt"); err == nil {
		ch <- fmt.Sprintf("Example output: %d\n", sum)
	} else {
		ch <- fmt.Sprintf("error processing part two example: %v\n", err)
	}

	if sum, err := d.ProcessPartTwo("./input/day2.txt"); err == nil {
...
func (d Day2) ProcessPartOne(name string) (int, error) {
	sum := 0
...
func (d Day2) ProcessPartTwo(name string) (int, error) {
	sum := 0

...
func (d Day4) PartOne(ch chan string) {
	...
	if sum, err := d.ProcessPartOne("./input/day4_example.txt"); err == nil {
		ch <- fmt.Sprintf("Example output: %d\n", sum)
	} else {
		ch <- fmt.Sprintf("error processing part one example: %v\n", err)
	}

	if sum, err := d.ProcessPartOne("./input/day4.txt"); err == nil {
...
func (d Day4) PartTwo(ch chan string) {
	...
	if sum, err := d.ProcessPartTwo("./input/day4_example.txt"); err == nil {
		ch <- fmt.Sprintf("Example output: %d\n", sum)
	} else {
		ch <- fmt.Sprintf("error processing part two example: %v\n", err)
	}

	if sum, err := d.ProcessPartTwo("./input/day4.txt"); err == nil {
...
func (d Day4) ProcessPartOne(name string) (int, error) {
	score := 0
...
func (d Day4) ProcessPartTwo(name string) (int, error) {
